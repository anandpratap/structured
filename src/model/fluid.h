...
template<class Tx, class Tad>
template<class Tq>
inline Tq FluidModel<Tx, Tad>::get_T_prho(const Tq p, const Tq rho){
	return p/rho/R;
};

template<class Tx, class Tad>
template<class Tq>
inline Tq FluidModel<Tx, Tad>::get_rho_pT(const Tq p, const Tq T){
	return p/T/R;
};
...
template<class Tx, class Tad>
template<class Tq>
inline Tq FluidModel<Tx, Tad>::get_p_rhoT(const Tq rho, const Tq T){
	return rho*R*T;
};

template<class Tx, class Tad>
template<class Tq>
inline Tq FluidModel<Tx, Tad>::get_laminar_viscosity(const Tq T){
	return mu_ref*pow(T/T_ref, 2.0/3.0);
};

template<class Tx, class Tad>
template<class Tq>
inline Tq FluidModel<Tx, Tad>::get_thermal_conductivity(const Tq T){
